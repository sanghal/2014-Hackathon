	...
	<head>
  	...
  	<body>
  		<?php
			...
			function get_bills_to_pay_name($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id){
			    ...
			    $sql = "SELECT * FROM user_bills WHERE payer_id=" . $user_id;
			    $result = mysql_query($sql);
			    $bills_to_pay_name = array();

			    while($row = mysql_fetch_assoc($result)){
			    	array_push($bills_to_pay_name, $row["title"]);
			    }

			    return $bills_to_pay_name;

			}

			function get_bills_to_pay_amount($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id){
			    ...
			    $sql = "SELECT * FROM user_bills WHERE payer_id=" . $user_id;
			    $result = mysql_query($sql);
			    $bills_to_pay_amount = array();

			    while($row = mysql_fetch_assoc($result)){
			    	array_push($bills_to_pay_amount, (float)$row["amount"]);
			    }

			    return $bills_to_pay_amount;

			}

			function get_bills_to_pay_owners($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id){
			    ...
			    $sql = "SELECT * FROM user_bills WHERE payer_id=" . $user_id;
			    $result = mysql_query($sql);
			    $bills_to_pay_owners = array();

			    while($row = mysql_fetch_assoc($result)){
			    	array_push($bills_to_pay_owners, (int)$row["recipient_id"]);
			    }

			    return $bills_to_pay_owners;
			}

			function get_bills_to_pay_id($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id){
			...
			function add_bill_form($user_id, $tenant_name_array, $tenant_id_array) {
  			...
  			function pay_bill($sql_hostname, $sql_username, $sql_password, $db_name, $bills_to_pay_name, $bills_to_pay_amount, $recipient_names, $bills_by_id){
				echo "<form method='post' action='pay_bill_submit.php'>";

				echo "  Select bill to pay:";
                echo "<select name='bill_selected'>";
                echo "<option value=''>Select...</option>";


            	for($i = 0; $i < sizeof($recipient_names); $i++){

            		$option_string = "From: " . $recipient_names[$i] . "  For: $" . $bills_to_pay_amount[$i] . " To cover: " . $bills_to_pay_name[$i];

                    echo "<option value=" . $bills_by_id[$i] ."> " . $option_string ." </option>";
                }
            
                echo '</select>';
				echo "<input type='submit' name='commit' value='Pay Bill'>";
			...
			echo sizeof($tenant_id_array);
  			...
  			$bills_to_pay_name = get_bills_to_pay_name($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id);
  			$bills_to_pay_amount = get_bills_to_pay_amount($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id);
  			$bills_to_pay_owners = get_bills_to_pay_owners($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id);


  			add_bill_form($user_id, $tenant_name_array, $tenant_id_array);
  			...
  			foreach ($bills_to_pay_owners as $owner){
  				echo $owner;
  				$name = get_name_from_id($sql_hostname, $sql_username, $sql_password, $db_name, $owner);
  				array_push($recipient_names, $name);
  			}

  			$bills_by_id = get_bills_to_pay_id($sql_hostname, $sql_username, $sql_password, $db_name, $user_id, $house_id);

  			pay_bill($sql_hostname, $sql_username, $sql_password, $db_name, $bills_to_pay_name, $bills_to_pay_amount, $recipient_names, $bills_by_id);
